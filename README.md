...
Ranked Results with Component Score Breakdown (example for 1 job description with 2 candidates' CVs)
...
Tesseract OCR — install here

Installation
bashgit clone https://github.com/alexander-drg/license_hospital_project.git
...
Make sure MySQL Server is running on your machine or is accessible on your network
In the app, navigate to the SQL Analyser section from the top menu
Enter your connection parameters directly in the interface:
...
Click Connect — the app will query the database and display all available tables
Select the tables you want to analyse and click Analyse Selected Tables

Usage

Upload CVs —> drag and drop or select PDF/DOCX/TXT/image files
Upload a Job Description —> same supported formats
Run Match —> click "Match Candidates" to get the ranked list with component scores
AI Analysis —> click "Generate AI Analysis" for Ollama-generated candidate summaries
SQL Analyser —> connect to your MySQL database, select tables, and get AI-assisted schema interpretation
...
Tech Stack
ComponentTechnologyBackendPython 3, FlaskFrontendHTML, CSS, Jinja2PDF Extractionpdfplumber, PyMuPDFOCRTesseract via pytesseractNLP / Embeddingssentence-transformers, OllamaDatabaseMySQL via mysql-connector-pythonML Utilitiesscikit-learn, numpy, scipy
